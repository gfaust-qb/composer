...
class Cvs
{
    ...
    /**
     * Execute an CVS command and try to fix up the process with credentials
     * if necessary.
     *
     * @param string $command CVS command to run
     * @param string $url     CVS url
    ...
    public function execute($command, $url, $cwd = null, $path = null, $verbose = false)
    {
        ...
        $cvsCommand = $this->getCommand($command, $url, $path);
        $output = null;
        $io = $this->io;
        ...
        $status = $this->process->execute($cvsCommand, $handler, $cwd);
        if (0 === $status) {
            return $output;
        }
        ...
        if (false === stripos($fullOutput, 'Could not authenticate to server:')
            ...
            && false === stripos($fullOutput, 'cvs: E170001:')
            && false === stripos($fullOutput, 'cvs: E215004:')) {
    ...
    protected function doAuthDance()
    {
        ...
        $this->io->writeError("The CVS server ({$this->url}) requested credentials:");

        $this->hasAuth = true;
        $this->credentials['username'] = $this->io->ask("Username: ");
        $this->credentials['password'] = $this->io->askAndHideAnswer("Password: ");

        $this->cacheCredentials = $this->io->askConfirmation("Should CVS cache these credentials? (yes/no) ", true);

        return $this;
    }

    /**
     * A method to create the cvs commands run.
     *
     * @param string $cmd  Usually 'cvs ls' or something like that.
    ...
    /**
     * Return the credential string for the cvs command.
     *
    ...
    /**
     * Get the password for the cvs command. Can be empty.
     *
    ...
    protected function getPassword()
    {
        if ($this->credentials === null) {
            throw new \LogicException("No cvs auth detected.");
        }
    ...
    /**
     * Get the username for the cvs command.
     *
     * @throws \LogicException
     * @return string
     */
